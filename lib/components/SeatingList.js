...
export const fetchData = async () => {
  ...
  try {
    const result = await getSeating({ finalData });
    if (result === "Not Available") {
      // console.log("create Data");
      const updateResult = await updateSeating({ finalData });
...
const getSeating = async ({ finalData }) => {
  return await getSeatingPlan({
    finalData,
  });
};

const updateSeating = async ({ finalData }) => {
